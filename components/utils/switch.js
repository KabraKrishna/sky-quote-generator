...
export default function Switch({ onToggle }) {
    ...
    return (
        <div className="relative w-11/12 flex items-center justify-between p-1 bg-white rounded-md">
            <input
                checked={toggleState}
                ref={toggleRef}
                ...
                onChange={() => {}}
                type="checkbox"
                className="z-20 absolute left-1/2 -translate-x-1/2 w-full appearance-none h-full peer rounded-md" />
            <div
                className="
            ...
            <div className="w-6/12 flex-col-center py-1 invisible peer-checked:visible duration-300 ease-in-out">
                <span className="text-sm text-indigo-500">{screens[1]}</span>
            </div>
            <div className="w-6/12 flex-col-center py-1 visible peer-checked:invisible duration-300 ease-in-out">
